...
grep -c 'icyd[\ \t]*es:' $XKB_ROOT/rules/$base.lst 1>/dev/null || \
sed -i '
/\!\ variant/a\
  icyd             es: IcyD Dvorak
...
COUNT=$(grep -c "<name>icyd</name>" $XKB_ROOT/rules/$base.xml) || true
if [ $COUNT -eq 0 ]; then
...
<xsl:template match="xkbConfigRegistry/layoutList/layout/variantList[preceding-sibling::configItem/name='es']">
  <xsl:copy>
    ...
    <xsl:text disable-output-escaping="yes"><![CDATA[  <variant>
          <configItem>
            <name>icyd</name>
            <description>IcyD Dvorak</description>
          </configItem>
        </variant>
      ]]></xsl:text>
  </xsl:copy>
...
grep -c 'xkb_symbols[\ \t]*"icyd"' $XKB_ROOT/symbols/es 1>/dev/null || \
